...
export class MelserOtpInput extends MelserBaseInput<string> {
    ...
    static styles = [
        ...
        css`
      :host {
        /* --- DEFINICIÓN DE VARIABLES (Theming) --- */
        /* Colores por defecto (Light Mode) */
        --otp-slot-bg: var(--melser-surface, #ffffff);
        --otp-slot-bg-filled: var(--melser-surface-highlight, #f8f9fa);
        --otp-slot-border: var(--melser-border, #e2e8f0);
        --otp-slot-text: var(--melser-text, #1a202c);
        
        /* Estado Activo (Focus) */
        --otp-active-border: var(--melser-primary, #3b82f6);
        --otp-active-shadow: rgba(59, 130, 246, 0.2);
        
        /* Cursor */
        --otp-caret-color: var(--melser-primary, #3b82f6);
      ...
      @media (prefers-color-scheme: dark) {
        :host {
          --otp-slot-bg: #1e293b;         /* Slate-800 */
          --otp-slot-bg-filled: #334155;  /* Slate-700 */
          --otp-slot-border: #475569;     /* Slate-600 */
          --otp-slot-text: #f8fafc;       /* Slate-50 */
          /* El primary suele servir para ambos modos, o se redefine aquí */
        }
      }
      ...
      .slot {
        ...
        background: var(--otp-slot-bg);
        border: 1px solid var(--otp-slot-border);
        color: var(--otp-slot-text);
      ...
      .slot.filled {
        background: var(--otp-slot-bg-filled);
      }

      .slot.active {
        border-color: var(--otp-active-border);
        box-shadow: 0 0 0 3px var(--otp-active-shadow);
      ...
      .fake-caret {
        ...
        background-color: var(--otp-caret-color);
      ...
      :host([invalid]) .slot {
        border-color: var(--melser-error, #ef4444);
        color: var(--melser-error, #ef4444);
      }
    `
    ];
}